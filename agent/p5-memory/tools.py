import os, ast, json, requests, subprocess, time
...
# Persistent long-term memory store — the capability that differentiates this
# harness ("Recall") from p4-agent-to-agent. Facts survive across sessions and
# process restarts, stored as one JSON array on disk (no vector index; recall
# does a plain substring/keyword match, good enough for the fact counts a
# single-user local harness will ever accumulate).
